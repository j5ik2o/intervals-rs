...
impl<T: Clone + Eq + Ord + PartialEq + PartialOrd> PartialOrd for IntervalLimit<T> {
    fn partial_cmp(&self, other: &Self) -> Option<Ordering> {
        if self.value.is_limitless() && other.value.is_limitless() {
            ...
            return self.lower_to_ordering(Some(Ordering::Less), Some(Ordering::Greater));
        }
        if self.value.is_limitless() {
            return self.lower_to_ordering(Some(Ordering::Less), Some(Ordering::Greater));
        }
        if other.value.is_limitless() {
            return other.lower_to_ordering(Some(Ordering::Greater), Some(Ordering::Less));
        }
        if self.value == other.value {
            if self.lower && other.lower {
                if self.closed ^ other.closed {
                    return self.closed_to_ordering(Some(Ordering::Less), Some(Ordering::Greater));
                }
                return Some(Ordering::Equal);
            }
            if !self.lower && !other.lower {
                if self.closed ^ other.closed {
                    return self.closed_to_ordering(Some(Ordering::Greater), Some(Ordering::Less));
                }
                return Some(Ordering::Equal);
            }
            return self.lower_to_ordering(Some(Ordering::Less), Some(Ordering::Greater));
...
impl<T: Clone + Eq + Ord + PartialEq + PartialOrd> IntervalLimit<T> {
    ...
    fn lower_to_ordering(&self, t: Option<Ordering>, f: Option<Ordering>) -> Option<Ordering> {
        if self.lower {
            t
        } else {
            f
    ...
    fn closed_to_ordering(&self, t: Option<Ordering>, f: Option<Ordering>) -> Option<Ordering> {
        if self.closed {
            t
        } else {
            f
...
impl<T: Display + Clone + Eq + Ord + PartialEq + PartialOrd> Display for IntervalLimit<T> {
    fn fmt(&self, f: &mut Formatter<'_>) -> std::fmt::Result {
        write!(f, "IntervalLimit({}, {}, {})", self.closed, self.lower, self.value)
    }
}
...
mod tests {
    ...
    #[test]
    fn test01_equals() {
        assert_eq!(IntervalLimit::lower(false, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::lower(true, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::lower(false, LimitValue::Limit(10)), IntervalLimit::lower(true, LimitValue::Limit(10)));
        assert_eq!(IntervalLimit::lower(true, LimitValue::Limit(10)), IntervalLimit::lower(true, LimitValue::Limit(10)));

        assert_eq!(IntervalLimit::upper(false, LimitValue::Limit(10)), IntervalLimit::upper(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::upper(true, LimitValue::Limit(10)), IntervalLimit::upper(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::upper(false, LimitValue::Limit(10)), IntervalLimit::upper(true, LimitValue::Limit(10)));
        assert_eq!(IntervalLimit::upper(true, LimitValue::Limit(10)), IntervalLimit::upper(true, LimitValue::Limit(10)));

        assert_ne!(IntervalLimit::lower(false, LimitValue::Limit(10)), IntervalLimit::upper(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::lower(true, LimitValue::Limit(10)), IntervalLimit::upper(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::lower(false, LimitValue::Limit(10)), IntervalLimit::upper(true, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::lower(true, LimitValue::Limit(10)), IntervalLimit::upper(true, LimitValue::Limit(10)));

        assert_ne!(IntervalLimit::upper(false, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::upper(true, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::upper(false, LimitValue::Limit(10)), IntervalLimit::lower(true, LimitValue::Limit(10)));
        assert_ne!(IntervalLimit::upper(true, LimitValue::Limit(10)), IntervalLimit::lower(true, LimitValue::Limit(10)));

        assert_eq!(IntervalLimit::new(false, false, LimitValue::Limit(1)), IntervalLimit::new(false, false, LimitValue::Limit(1)));

        assert_eq!(IntervalLimit::lower(false, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
        assert_eq!(IntervalLimit::new(false, true, LimitValue::Limit(10)), IntervalLimit::lower(false, LimitValue::Limit(10)));
    }
    ...
    fn test03_sort() {
        ...
        assert_eq!(list.get(0).unwrap(), &IntervalLimit::lower(false, LimitValue::Limitless));
        assert_eq!(list.get(1).unwrap(), &IntervalLimit::lower(false, LimitValue::Limitless));

        assert_eq!(list.get(2).unwrap(), &IntervalLimit::lower(true, LimitValue::Limit(1)));
        assert_eq!(list.get(3).unwrap(), &IntervalLimit::lower(false, LimitValue::Limit(1)));
        assert_eq!(list.get(4).unwrap(), &IntervalLimit::upper(false, LimitValue::Limit(1)));
        assert_eq!(list.get(5).unwrap(), &IntervalLimit::upper(true, LimitValue::Limit(1)));
        assert_eq!(list.get(6).unwrap(), &IntervalLimit::lower(true, LimitValue::Limit(5)));
        assert_eq!(list.get(7).unwrap(), &IntervalLimit::lower(false, LimitValue::Limit(5)));
        assert_eq!(list.get(8).unwrap(), &IntervalLimit::upper(false, LimitValue::Limit(5)));
        assert_eq!(list.get(9).unwrap(), &IntervalLimit::upper(true, LimitValue::Limit(5)));

        assert_eq!(list.get(10).unwrap(), &IntervalLimit::upper(false, LimitValue::Limitless));
        assert_eq!(list.get(11).unwrap(), &IntervalLimit::upper(false, LimitValue::Limitless));
